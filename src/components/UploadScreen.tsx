...
import { canStartUpload, cccdRequirementMessage } from '../upload/cccd'
      ...
      <div className="upload-card">
        ...
        <label className="dropzone dropzone-sm">
          <input
            ...
            accept=".xlsx"
            className="dropzone-input"
            onChange={e => setCccd(e.target.files?.[0] ?? null)}
          />
          <span className="dropzone-icon">🪪</span>
        ...
        {validationMessage && <p className="upload-validation" role="alert">{validationMessage}</p>}

        <button
          type="button"
          className="btn primary upload-start"
          disabled={!canStart}
          onClick={() => pdf && onStart(pdf, roster ?? undefined, cccd ?? undefined)}
        >
