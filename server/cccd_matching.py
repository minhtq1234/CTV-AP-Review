"""Exact-only roster resolution for local CCCD card candidates."""
...
ResolutionState: TypeAlias = Literal[
    "exact",
    "suggested",
    "manual",
    "conflict",
...
def resolve_candidates(
    ...
    by_cccd = _index_many(
        roster_rows,
        lambda row: _digits(row.get("cccd", "")),
    )
    by_name = _index_many(
        roster_rows,
        lambda row: _name_key(row.get("name", "")),
    )
    ...
    return ResolutionResult(
        expected_mappable_identities=expected,
...
def _candidate_claims(candidate, by_cccd, by_name) -> set[int]:
    ...
    name_matches = (
        by_name.get(name, [])
        if ocr.name_confidence >= _SUGGESTED_NAME_CONFIDENCE
        else []
    )
    if (
        len(name_matches) == 1
        and _is_unique_eligible_identity(name_matches[0][0], by_cccd)
    ):
...
def _is_unique_eligible_identity(index: int, by_cccd) -> bool:
    for cccd, matches in by_cccd.items():
        if (
            len(cccd) == 12
            and len(matches) == 1
            and matches[0][0] == index
        ):
            return True
    return False


def _claims_by_target(
    claims_by_candidate: dict[str, set[int]],
) -> dict[int, set[str]]:
...
def _resolve_one(
    ...
    if (
        cccd_target is not None
        and _roster_name_is_duplicate(cccd_target, roster_rows, by_name)
    ):
        return _conflict(candidate.id, issues, "duplicate-name")
    if (
        cccd_target is not None
        and name_target is not None
        and cccd_target != name_target
    ):
    ...
    if (
        cccd_target is not None
        and ocr.cccd_confidence >= _AUTO_CCCD_CONFIDENCE
    ):
        return _resolved(
            candidate.id,
            "exact",
    ...
    if (
        ...
        and _is_unique_eligible_identity(name_target, by_cccd)
    ):
        return _resolved(
            candidate.id,
            "suggested",
    ...
    if len(name_matches) == 1:
        ...
        if _is_unique_eligible_identity(target, by_cccd):
            return _resolved(
                ...
                target,
                "name",
                _with_issue(issues, "no-number-region"),
            )
