"""Deterministic, conservative front/back pairing for the local CCCD spike."""
...
def pair_drawings(images: list[AnalyzedDrawing]) -> list[CardCandidate]:
    """Pair only unambiguous mutual-nearest front and back drawings.

    Every input is represented exactly once in the result.  Known but
    unpaired sides keep a missing-side or ambiguity issue; unknown sides stay
    available for manual handling without being promoted to either card side.
    """
    ...
    eligible_backs = {front.drawing.id: _eligible_opposites(front, backs) for front in fronts}
    eligible_fronts = {back.drawing.id: _eligible_opposites(back, fronts) for back in backs}
    ...
    for front in fronts:
        ...
        if (
            ...
            and _has_margin(choices[0][0], [distance for distance, _ in choices[1:]])
            and _has_margin(reverse_choices[0][0], [distance for distance, _ in reverse_choices[1:]])
...
def _unpaired_candidate(
    ...
    if image.ocr.side == "front":
        issue = "ambiguous-pair" if eligible_backs[image.drawing.id] else "missing-back"
        return CardCandidate(f"card-{image.drawing.id}", image, None, (issue,))
    if image.ocr.side == "back":
        issue = "ambiguous-pair" if eligible_fronts[image.drawing.id] else "missing-front"
        return CardCandidate(f"card-{image.drawing.id}", None, image, (issue,))
    return CardCandidate(f"card-{image.drawing.id}", None, None, ("unknown-side",))


def _eligible_opposites(
    source: AnalyzedDrawing, candidates: list[AnalyzedDrawing]
) -> list[tuple[float, AnalyzedDrawing]]:
    return sorted(
        (
            (_anchor_center_distance(source.drawing.anchor, candidate.drawing.anchor), candidate)
...
def _eligible(first: AnalyzedDrawing, second: AnalyzedDrawing) -> bool:
    ...
    return overlap >= 0.5 or row_delta <= 1
...
def _vertical_overlap_ratio(first: Anchor, second: Anchor) -> float:
    ...
    if first_height <= 0 or second_height <= 0:
        return 0.0
    overlap = max(0, min(first.to_row, second.to_row) - max(first.from_row, second.from_row))
    return overlap / min(first_height, second_height)


def _anchor_center_distance(first: Anchor, second: Anchor) -> float:
    """Measure center distance in each drawing's own grid-scale units.

    Rows and columns are not comparable physical units in a worksheet.  The
    average anchor spans normalize their center offsets, so a one-row jitter
    within a tall card does not outweigh a full-card horizontal displacement.
    """
    ...
    row_scale = max(1.0, ((first.to_row - first.from_row) + (second.to_row - second.from_row)) / 2)
    col_scale = max(1.0, ((first.to_col - first.from_col) + (second.to_col - second.from_col)) / 2)
    return hypot((first_row_center - second_row_center) / row_scale, (first_col_center - second_col_center) / col_scale)


def _has_margin(best: float, alternatives: list[float]) -> bool:
    return not alternatives or (best < min(alternatives) and best <= min(alternatives) * 0.8)
