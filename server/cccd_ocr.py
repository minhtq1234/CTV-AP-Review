...
def _group_words_into_lines(words: list[OcrWord], y_tolerance: int = 8) -> list[list[OcrWord]]:
    """Group OCR words by baseline and sort each line from left to right."""
...
def _contains_phrase(words: list[OcrWord], phrase: str) -> tuple[bool, float]:
    ...
    for start in range(len(tokens) - len(wanted) + 1):
        if tokens[start:start + len(wanted)] == wanted:
            confidence = min(word.confidence for word in words[start:start + len(wanted)])
...
def classify_side(words: list[OcrWord]) -> tuple[Side, float]:
    """Classify a card only when at least two independent marker groups agree."""
    front = _marker_groups(words, _FRONT_MARKERS)
    back = _marker_groups(words, _BACK_MARKERS)
    if len(front) < 2 and len(back) < 2:
        return "unknown", 0.0
    if len(front) == len(back):
        return "unknown", 0.0
    markers = front if len(front) > len(back) else back
    return ("front" if len(front) > len(back) else "back"), min(markers)


def _number_label_span(line: list[OcrWord]) -> tuple[int, int] | None:
    """Return the word-index span of a printed CCCD number label."""
...
def _words_right_of(
    line: list[OcrWord], label: tuple[int, int], min_confidence: float,
) -> list[OcrWord]:
    label_right = max(word.x + word.width for word in line[label[0]:label[1]])
    return [
        word for word in line[label[1]:]
        if word.x >= label_right and word.confidence >= min_confidence and _digits(word)
    ]


def _next_line_digits(lines: list[list[OcrWord]], line_index: int) -> list[OcrWord]:
    if line_index + 1 >= len(lines):
        return []
    return [word for word in lines[line_index + 1] if word.confidence >= .5 and _digits(word)]


def _padded_union(
    words: list[OcrWord], image_width: int, image_height: int, pad: int,
) -> dict[str, int]:
    x0 = max(0, min(word.x for word in words) - pad)
    y0 = max(0, min(word.y for word in words) - pad)
    x1 = min(image_width, max(word.x + word.width for word in words) + pad)
    y1 = min(image_height, max(word.y + word.height for word in words) + pad)
...
def locate_number_region(
    ...
    """Locate digits only when they are anchored to a CCCD number label."""
...
def _upright_image(path: str) -> Image.Image:
    """Load a local image, respecting EXIF and confident local OSD rotation."""
    with Image.open(path) as source:
        image = ImageOps.exif_transpose(source).convert("RGB")
    try:
        osd = pytesseract.image_to_osd(image, output_type=pytesseract.Output.DICT)
        rotation = _upright_rotation(
            int(osd.get("rotate", 0)), float(osd.get("orientation_conf", 0.0)),
        )
    except Exception:  # Sparse images can make local Tesseract OSD fail.
        rotation = 0
    return image.rotate(rotation, expand=True) if rotation else image


def _full_image_words(image: Image.Image) -> list[OcrWord]:
    """Perform the sole full-image OCR pass used for labels and geometry."""
...
def _region_digits(image: Image.Image, bbox: dict[str, int]) -> tuple[str, float]:
    """OCR only the label-anchored number crop using a digits-only setting."""
    crop = image.crop((
    ...
    if not readable:
        return "", 0.0
    return "".join(text for text, _ in readable), min(confidence for _, confidence in readable)
...
def analyze_drawing(drawing: EmbeddedDrawing) -> CccdImageOcr:
    """Analyze one extracted drawing without ever OCRing whole-image digits."""
